...
def get_dns_ips():
    ...
    ips = [r["content"] for r in records if r.get("type") == "A" and r.get("content")]

    if not ips:
        raise RuntimeError(f"No A records found for {CF_DNS_NAME}")
...
def write_files(ips):
    ...
    for index, ip in enumerate(ips, start=1):
        ...
        <tr>
            <td>{index}</td>
            <td><a href="https://zh-hans.ipshu.com/ipv4/{escape(ip)}" target="_blank">{escape(ip)}</a></td>
            <td>{escape(CF_DNS_NAME)}</td>
            <td>{now}</td>
        </tr>
        """
...
<head>
    ...
    <title>我的 Cloudflare 优选 IP</title>
    <style>
        body {{
...
def main():
    ...
    print(f"Generated pages for {CF_DNS_NAME}: {', '.join(ips)}")
